...
$(this).parent().parent().parent().find("#tableofscraption tbody").append('<tr ><td id="Target">'+Target+'</td><td id="species">'+species+'</td><td id="Ki">'+Ki+'</td><td id="IC50">'+IC50+'</td><td id="kd">'+kd+'</td><td id="EC50">'+EC50+'</td></tr>')
});
...
var fatchedall = window.location.href.split(/Increment=/);
var fatched = fatchedall[1];
...
$( "#tableofscraption tbody tr:contains('(Homo sapiens (Human))')" ).each(function( index ) {
$(this).attr('id', 'Human');
})
//* then loop on none human study
$( "#tableofscraption tbody tr:not(#Human)" ).each(function( index ) {
...
$("#tableofscraption tbody tr:not(#Human) #Target:contains('"+nonhumantaregt+"')").parent().remove();
}
}
)
//* changing non human button to done
$('#clickMe').attr("value","done");
};